...
def test_branch_score_and_matrix():
    ...
    assert first.branch_score(second) == pytest.approx(expected)
    assert second.branch_score(first) == pytest.approx(expected)

    trees = TreeCollection.from_newick(["(A:1,B:2);", "(B:4,A:2);"])
    distances = branch_score_matrix(trees)
    assert distances.dtype == np.float64
    assert distances.shape == (2, 2)
    assert np.allclose(distances, [[0.0, expected], [expected, 0.0]])
    assert not distances.flags.writeable


def test_clade_frequencies():
    ...
    assert clade_frequencies(trees) == {
        frozenset({"A", "B"}): 2 / 3,
        frozenset({"C", "D"}): 2 / 3,
        frozenset({"A", "C"}): 1 / 3,
        frozenset({"B", "D"}): 1 / 3,
    }
    assert clade_frequencies(TreeCollection.from_newick([])) == {}
