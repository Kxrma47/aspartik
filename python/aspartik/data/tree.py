...
def branch_score_matrix(trees: TreeCollection):
    import numpy as np

    return np.frombuffer(trees._branch_score_matrix(), dtype=np.float64).reshape(
        (len(trees), len(trees))
    )


def clade_frequencies(trees: TreeCollection):
    return {
        frozenset(clade): frequency for clade, frequency in trees._clade_frequencies()
    }
